...
float ColorPixel(float cur_col, float c) {
    if (cur_col <= c) {
        return 0;
    } else {
        return 1;
...
Image FilterED(Image& copy, float threshold) {
    FilterGrayscale().UseFilter(copy, {});
    ...
    for (size_t i = 0; i < copy.height_; ++i) {
        for (size_t j = 0; j < copy.width_; ++j) {
            Color col;
            float b = std::max(
                0.0f, (-1) * (vec[i][j + 1].b_ + vec[i + 1][j].b_ + vec[i + 1][j + 2].b_ + vec[i + 2][j + 1].b_) +
                          vec[i + 1][j + 1].b_ * (a));
            float r = std::max(
                0.0f, (-1) * (vec[i][j + 1].r_ + vec[i + 1][j].r_ + vec[i + 1][j + 2].r_ + vec[i + 2][j + 1].r_) +
                          vec[i + 1][j + 1].r_ * (a));
            float g = std::max(
                0.0f, (-1) * (vec[i][j + 1].g_ + vec[i + 1][j].g_ + vec[i + 1][j + 2].g_ + vec[i + 2][j + 1].g_) +
                          vec[i + 1][j + 1].g_ * (a));

            col.b_ = ColorPixel(b, threshold);
            col.r_ = ColorPixel(r, threshold);
            col.g_ = ColorPixel(g, threshold);
...
void FilterEdgeDetection::UseFilter(Image& image, std::vector<std::string> vec) {
    float t = std::stof(vec[0]);
    FilterED(image, t);
}